...
#check if there's NA in c_reactive_protein column
sum(is.na(df$c_reactive_protein)) # 3 NAs.

#omit NA
df <- na.omit(buss3 %>% filter(BMI.weight == "Healthy") %>% select(TPTG:H4A2, c_reactive_protein)) #519
...
#creating heatmap
corrplot(M, method="circle", cl.ratio = 0.2, tl.srt = 45)
