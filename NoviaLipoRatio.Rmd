...
# Load Data
```{r}
bus_sau2 <- read.csv("/Users/novia/Desktop/Diabetes_Obesity/Busselton/BusT1_Sau_Healthy_DM2_28112023.csv")
names(bus_sau2)
```
...
ann_buss <- read.csv("/Users/novia/Desktop/Diabetes_Obesity/Busselton/Anno_t1_MH_new.csv", header=TRUE)
dim(ann_buss) #1976 obs

#loading buss lipo from Reika
buss <- read.csv(file='/Users/novia/Desktop/Diabetes_Obesity/Busselton/Busselton_Lipo_290523.csv') 
dim(buss) #4292 obs
...
#write.csv(buss2, file = "Matched_Bus_anno_lipo_allcolumns_NM_13052024.csv", row.names = F)
...
#write.csv(buss3, file = "Matched_Bus_anno_lipo_extendedversion_NM_13052024.csv", row.names = F)
...
#write.csv(buss4, file = "Matched_Bus_anno_lipo_w_CRP_Glyc_NM_14052024.csv", row.names=F)

buss4 <- read.csv("/Users/novia/Desktop/Diabetes_Obesity/Busselton/Matched_Bus_anno_lipo_w_CRP_Glyc_NM_14052024.csv", header=TRUE)
```
...
#23May2024 - all weights
#renaming Healthy BMI category to Healthy-weight
```{r}
buss4$BMI.weight[which(buss4$BMI.weight == "Healthy")] = "Healthy-weight"

#save(buss4, file="Busselton_Final_forPaper2_23May2024.rda")

load("Busselton_Final_forPaper2_23May2024.rda")
```

##Fig1. Corr heatmap for selected lipo
```{r}
df <- na.omit(buss4 %>% select(TPTG:LDPN, VLTG:LDAB, CRP, Glyc))

M <- cor(df)

corrplot(M, method="circle", type = "upper", diag = FALSE, cl.ratio = 0.2, tl.cex = 0.6, mar=c(0,0,1,0), title = "Busselton - all weights", col=colorRampPalette(c("blue","white","red"))(200))

```

##Fig2. Boxplots for selected lipo
```{r}
my_comparisons <- list( c("Healthy-weight", "Overweight"), c("Healthy-weight", "Obese"),c("Overweight", "Obese"))

buss5 <- buss4 %>% filter(BMI.weight != "Underweight")

buss5$BMI.weight <- factor(buss5$BMI.weight , levels=c("Healthy-weight", "Overweight", "Obese"))
```

### TPTG
```{r}
p1= ggboxplot(buss5, x = "BMI.weight", y = "TPTG",
           color = "BMI.weight", fill = "BMI.weight", alpha=0.4) + 
  scale_color_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) +
  scale_fill_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) + 
...
### VLPN
```{r}
p2= ggboxplot(buss5, x = "BMI.weight", y = "VLPN",
           color = "BMI.weight", fill = "BMI.weight", alpha=0.4) + 
  scale_color_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) +
  scale_fill_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) + 
  stat_compare_means(comparisons = my_comparisons, label="p.signif") # Add pairwise comparisons p-value
...
### IDPN
```{r}
p3= ggboxplot(buss5, x = "BMI.weight", y = "IDPN",
           color = "BMI.weight", fill = "BMI.weight", alpha=0.4) + 
  scale_color_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) +
  scale_fill_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) + 
  stat_compare_means(comparisons = my_comparisons, label="p.signif") # Add pairwise comparisons p-value
...
### LDPN
```{r}
p4= ggboxplot(buss5, x = "BMI.weight", y = "LDPN",
           color = "BMI.weight", fill = "BMI.weight", alpha=0.4) + 
  scale_color_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) +
  scale_fill_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) + 
  stat_compare_means(comparisons = my_comparisons, label="p.signif") # Add pairwise comparisons p-value
...
### VLAB
```{r}
p5= ggboxplot(buss5, x = "BMI.weight", y = "VLAB",
           color = "BMI.weight", fill = "BMI.weight", alpha=0.4) + 
  scale_color_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) +
  scale_fill_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) + 
  stat_compare_means(comparisons = my_comparisons, label="p.signif") # Add pairwise comparisons p-value
...
### IDAB
```{r}
p6= ggboxplot(buss5, x = "BMI.weight", y = "IDAB",
           color = "BMI.weight", fill = "BMI.weight", alpha=0.4) + 
  scale_color_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) +
  scale_fill_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) + 
  stat_compare_means(comparisons = my_comparisons, label="p.signif") # Add pairwise comparisons p-value
...
### LDAB
```{r}
p7= ggboxplot(buss5, x = "BMI.weight", y = "LDAB",
           color = "BMI.weight", fill = "BMI.weight", alpha=0.4) + 
  scale_color_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) +
  scale_fill_manual(values = c("Healthy-weight"="green4","Overweight" = "orange2","Obese"="red3")) + 
  stat_compare_means(comparisons = my_comparisons, label="p.signif") # Add pairwise comparisons p-value
...
```{r}
ggarrange(plot1,plot2,plot3,plot4,plot5,plot6, plot7, widths = c(1,1), common.legend = TRUE, legend = "bottom")

```


##Fig3. Parallel coordinates chart 
```{r}
tdf <- buss5%>%select(TPTG:H4A2, BMI.weight)

tdf$BMI.weight <- factor(buss5$BMI.weight , levels=c("Healthy-weight", "Overweight", "Obese"))

lcols <- c("#00BFC4", "#F8766D", "#FFC425")

table(tdf$BMI.weight)
...
for (l in lipo_name) {
  tdf1 <-
    data.frame(
      Healthy_weight = apply(na.omit(
        tdf %>% filter(BMI.weight == "Healthy-weight") %>% select(contains(l))
      ), 2, median),
      Overweight = apply(na.omit(
        tdf %>% filter(BMI.weight == "Overweight") %>% select(contains(l))
      ), 2, median),
      Obese = apply(na.omit(
       tdf %>% filter(BMI.weight == "Obese") %>% select(contains(l))
      ), 2, median)
    )
...
library(gridExtra)
grid.arrange(p1[[1]],p1[[2]],p1[[3]],p1[[4]],p1[[5]],p1[[6]],p1_legend, top=("Busselton"), nrow=3) 

ggarrange(p1[[1]],p1[[2]],p1[[3]],p1[[4]],p1[[5]],p1[[6]], widths = c(1,1), common.legend = TRUE, legend = "bottom", nrow = 6)

```


##Fig4. Corr heatmap TPTG, ABA1, Glyc + HDL subfraction
```{r}
df1 <- na.omit(buss5 %>% select(TPTG, ABA1, H1TG:H4A2, Glyc, CRP))

M1 <- cor(df1)

corrplot(M1, method="circle", type = "upper", diag = FALSE, cl.ratio = 0.2, tl.cex = 0.6, mar=c(0,0,1,0), title = "Busselton - all weights", col=colorRampPalette(c("blue","white","red"))(200))
