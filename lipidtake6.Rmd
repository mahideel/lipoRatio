...
Participants who are non-diabetic (and did not have any other conditions  (Summary_Healthy == 1)) and in healthy-weight BMI range. 

- Obese 

Participants who are non-diabetic (but may have other conditions) and in overweight to obese BMI range. 
...
## PCA
```{r,echo=FALSE, warning=FALSE,fig.width=10,fig.height=10,eval=FALSE}
library(gghalves)
library(cowplot)
library(ggforce)
...
# data.frame(Combined_subData%>%dplyr::select(Group,sex),mod1$data$scores)%>%
#   pivot_longer(cols = !Group:sex,
#                names_to = "PC",
#                values_to = "score")%>%
#   ggplot(aes(x = Group, y = score))+
#   geom_half_violin(aes(x = Group,y = score),side = "l",position = "identity")+
#   geom_half_violin(aes(x = Group,y = score,fill = sex,alpha = 0.5),side = "r",position = "identity")+
#   theme_bw()+
#     facet_wrap_paginate(~PC,scales = "free_y")
...
#l1<-plotLoadings(mod1,optns = list(PCi = i))
# l1<-plotLoadings(mod1)
# data.frame(lipid_names, x = l1$data$loadings[, i])%>%
#      mutate(class = factor(sapply(strsplit(lipid_names,".",fixed = TRUE),"[",1)))%>%
#      ggplot(aes(x = x,y = class,label =lipid_names))+
#      geom_text(size = 4,angle = 60)+
#      labs(title ="", subtitle = "Loadings", x =  paste0("PC ",i, "(",round(mod1$data$pcSum$`Proportion of Variance`[i],1),"%)"),y = "Lipid Class",caption = "")+
#      theme(
#        plot.background = element_rect(fill = "transparent", colour = NA),
#        plot.margin=unit(c(0,0,0,0), "pt"),
#        axis.text.y = element_text(size = 6,color = "black",family = "Times New Roman")
#        )+
#      theme_bw()+theme(legend.position = "bottom")->l1
# 
# p[[i]]<-plot_grid(s1,l1,align = "v", nrow = 2,rel_heights = c(1/3,2/3))
