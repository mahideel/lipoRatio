...
for (l in lipo_name) {
  ...
  tdf1 <- data.frame(
    Group1 = apply(na.omit(
      tdf %>%
        dplyr::filter(BMI_cat == "<25" & statin == 1) %>%
        select(all_of(selected_vars))
    ), 2, median),
    Group2 = apply(na.omit(
        tdf %>% 
          dplyr::filter(BMI_cat == "25-30" & statin == 1) %>% 
          select(all_of(selected_vars))
      ), 2, median),
    Group3 = apply(na.omit(
       tdf %>% 
         dplyr::filter(BMI_cat == ">30" & statin == 1) %>% 
         select(all_of(selected_vars))
      ), 2, median)
    )
...
annotate_figure(final_plot, top = text_grob("Busselton_STATIN", color = "black", face = "bold", size = 15))
...
for (l in lipo_name) {
  ...
  tdf1 <- data.frame(
    Group1 = apply(na.omit(
      tdf %>%
        dplyr::filter(BMI_cat == "<25" & statin == 0) %>%
        select(all_of(selected_vars))
    ), 2, median),
    Group2 = apply(na.omit(
        tdf %>% 
          dplyr::filter(BMI_cat == "25-30" & statin == 0) %>% 
          select(all_of(selected_vars))
      ), 2, median),
    Group3 = apply(na.omit(
       tdf %>% 
         dplyr::filter(BMI_cat == ">30" & statin == 0) %>% 
         select(all_of(selected_vars))
      ), 2, median)
    )
...
# 
# tdf <- rbind(tdf1, tdf2)
tdf <- tdf1 # just want to see statin vs no statin in Busselton 
...
```{r}
summarised_data <- tdf %>%
  dplyr::filter(cohort == "Busselton" & statin == 1 & comb %in% c("Hw_NDM", "Ob_NDM", "Hw_DM", "Ob_DM")) %>%
  group_by(comb) %>%
  summarise(med_LDPN = median(LDPN, na.rm = TRUE), med_VLPN = median(VLPN, na.rm=TRUE), 
            med_IDPN = median(IDPN, na.rm = TRUE), med_HDA1 = median(HDA1, na.rm = TRUE), 
            med_HDPL = median(HDPL, na.rm = TRUE), 
...
## Busselton - Not taking Statin
```{r}
summarised_data <- tdf %>%
  dplyr::filter(cohort == "Busselton" & statin == 0 & comb %in% c("Hw_NDM", "Ob_NDM", "Hw_DM", "Ob_DM")) %>%
  group_by(comb) %>%
  summarise(med_LDPN = median(LDPN, na.rm = TRUE), med_VLPN = median(VLPN, na.rm=TRUE), 
            med_IDPN = median(IDPN, na.rm = TRUE), med_HDA1 = median(HDA1, na.rm = TRUE), 
            med_HDPL = median(HDPL, na.rm = TRUE), 
